...
from .timeutil import eastern_today
...
class Pipeline:
    ...
    async def fetch_daily(
        ...
        try:
            ...
            fetch_id = await self._blocking(
                lambda: self.archive.record_fetch(
                    KIND_DAILY, ok=False, http_status=status, error=message
                )
            )
            _LOGGER.error("daily fetch failed (fetch %s): %s", fetch_id, message)
            raise
        ...
        try:
            ...
            fetch_id = await self._blocking(
                lambda: self.archive.record_fetch(
                    KIND_DAILY, ok=False, http_status=200, error=message, raw_html=page
                )
            )
            _LOGGER.error(
                "daily page did not parse; raw page kept as fetch %s: %s", fetch_id, message
            )
            raise
        ...
        fetch_id = await self._blocking(
            lambda: self.archive.record_fetch(
                ...
                window_start_utc=min(froms) if froms else None,
                window_end_utc=max(froms) if froms else None,
            )
        )
        store = await self._blocking(
        ...
        _LOGGER.info(
            "daily: fetch %s stored %s new, %s updated, %s unchanged (%s revisions)",
            fetch_id, store.inserted, store.updated, store.unchanged, store.revisions,
        )
        return FetchOutcome(
            kind=KIND_DAILY, fetch_id=fetch_id, ok=True, store=store, meters=daily.meters
        )

    # -- the daily cycle

    async def run_cycle(self, *, meter: str | None = None) -> CycleResult:
        ...
        return CycleResult(
            ...
            issues=tuple(f"{i.kind}: {i.detail}" for i in issues),
        )
